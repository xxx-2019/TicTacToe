...
public class Game {
    ...
    public String checkGameWinner(char [][]grid){
        ...
        else if (czyWygral(grid,'o')) {
            result = "O wins";
        }
        else if (czyKoniec(grid)) {
            result = "Tie";
        }
    ...
    public boolean czyWygral(char [][]grid, char who) {
        boolean ret = false;

        // Sprawdzenie wierszy
        for ( int i = 0; i < 3; i++) {
            if (grid[0][i] == who && grid[1][i] == who && grid[2][i] == who) {
                return true;
            }
        }
    ...
    public boolean czyKoniec(char [][]grid) {
        boolean ret = true;
        for (int i = 0; i < 3; i++)
            for (int j = 0; j < 3; j++)
                if (grid[i][j] == '-') {
